...
async function getCategoryCode() {
  ...
  if (!mainCategory) {
    ...
    if (higher.length > 0) {
      ...
      if (subCategory) {
        ...
        const higherID = allCategory.find(
          (item) => item._id === lowerID.parent,
        ); // 하위 카테고리에 맞는 상위 카테고리를 찾는다.
  ...
  if (!subCategory) {
    ...
    const lower = allCategory.filter(
      (item) => item.parent && higherId._id === item.parent,
    );
...
async function makeTabButtons(tabs) {
  ...
  list.forEach((tab) => {
    ...
    subbtn.href =
      "/products?higherCategory=" + mainCategory + "&lowerCategory=" + tab.name;
...
document
  ...
  .addEventListener("change", changeSortType);
...
// 페이징 생성 메서드
function makePagenation(totalCnt) {
  // (Number) 총 상품 갯수(=totalCnt)
